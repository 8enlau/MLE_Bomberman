...
class AgentRunner:
    ...
    def __init__(self, train, agent_name, code_name, result_queue):
        ...
        self.callbacks = importlib.import_module('create_Dataset.agent_code.' + self.code_name + '.callbacks')
        if train:
            self.train = importlib.import_module('create_Dataset.agent_code.' + self.code_name + '.train')
